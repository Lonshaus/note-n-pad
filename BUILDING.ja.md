...
同じバージョンであれば、機能はストア版とまったく同じです。違いは 2 点だけです。ストア版は署名と公証を済ませているため OS にブロックされませんが、自分でビルドしたものは未署名なので、初回起動時に一手間余計にかかります。また自動更新には対応していないため、バージョンが上がるたびに再度ビルドし直す必要があります。
